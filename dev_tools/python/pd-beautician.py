...
def main():
    ...
    if not allTexts or (not inlets and not outlets):
        eprint("nothing to show, pass.")
        return 0

    # GraphOnParent dimensions
    gopWidth = max(
        len(title) * 7 + 16, LABEL_WIDTH * max(len(inlets), len(outlets)) + 10
    )
    ...
    # beautify inlets
    labelDeltaX = (
        ((gopWidth - LABEL_WIDTH) / (len(inlets) - 1)) if len(inlets) > 1 else 0
    )
    socketDeltaX = (
        ((gopWidth - SOCKET_WIDTH) / (len(inlets) - 1)) if len(inlets) > 1 else 0
    )
    for i, inlet in enumerate(sorted(inlets, key=byX)):
        ...
        for text in allTexts:
            if text.inRect(
                inlet.x - MAX_TEXT_DISTANCE,
                inlet.x + MAX_TEXT_DISTANCE,
                inlet.y - MAX_TEXT_DISTANCE,
                inlet.y,
            ):
                comment = text.value
        ...
        patch.addDef(
            cnvLabel(gopLeft + labelDeltaX * i, gopTop + SOCKET_HEIGHT, comment)
        )

    # set title
    patch.addDef(
        Canvas(
            x=gopLeft + (gopWidth - len(title) * 7) / 2,
    ...
    lastTop = argsTop
    if patch.getDefOfType("\$1") or any(
        [
            "\$1" in str(arg)
        ...
        patch.addDef(
            Canvas(
                x=gopLeft + 5,
                y=gopTop + LABEL_HEIGHT + TITLE_HEIGHT + 10,
                width=gopWidth - 10,
                height=20,
                boxSize=1,
                textSize=12,
        ...
        patch.chainConnect(
            [
                patch.addDef(Object(gopLeft, argsTop + 10, type="loadbang")),
    ...
    # beautify outlets
    labelDeltaX = (
        ((gopWidth - LABEL_WIDTH) / (len(outlets) - 1)) if len(outlets) > 1 else 0
    )
    socketDeltaX = (
        ((gopWidth - SOCKET_WIDTH) / (len(outlets) - 1)) if len(outlets) > 1 else 0
    )
    for i, outlet in enumerate(sorted(outlets, key=byX)):
        ...
        for text in allTexts:
            if text.inRect(
                outlet.x - MAX_TEXT_DISTANCE,
                outlet.x + MAX_TEXT_DISTANCE,
                outlet.y,
                outlet.y + MAX_TEXT_DISTANCE,
            ):
                comment = text.value

        patch.addDef(
            cnvSocket(gopLeft + socketDeltaX * i, gopTop + gopHeight - SOCKET_HEIGHT)
        )
        patch.addDef(
            cnvLabel(
                gopLeft + labelDeltaX * i,
    ...
    # move all objects at coordinates >0
    objectList = patch.definitions + patch.subPatches

    # find minimal coordinates
    xmin = min([obj.x for obj in objectList])
    ymin = min([obj.y for obj in objectList])
